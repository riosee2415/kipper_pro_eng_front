...
import AD209Presenter from "./AD209Presenter";
import { useMutation, useQuery } from "@apollo/client";
import { REGISTER_PRODUCT, CREATE_COLORIMAGES } from "./AD209Queries.js";
...
export default ({ history }) => {
  ...
  ////////////// - USE MUTATION- //////////////
  const [registerProductMutation] = useMutation(REGISTER_PRODUCT);
  ...
  const registerProduct = () => {
    confirm(
      `제품 등록`,
      `해당 제품을 등록하시겠습니까?`,
      registerProductAfter,
      null
    );
  };

  const registerProductAfter = async () => {
    const tempArr = colorImageList.map((data) => {
      return data._id;
    });
    const { data } = await registerProductMutation({
      variables: {
    ...
    if (data.registerProduct) {
      toast.info("제품 등록 되었습니다.");
